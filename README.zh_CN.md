...
docker run \
  ...
  -e password=11****11 \
  -e appid=f7af****ebb \
  -v /volume1/docker/AutoSignMachine/***.log:/AutoSignMachine/***.log \
  -v /volume1/docker/AutoSignMachine/crontab***:/etc/crontabs/root \
  -v /volume1/docker/AutoSignMachine/user/:/root/.AutoSignMachine/ \
  auto-sign-machine:latest
```
进入容器后需要手动运行下命令

```
docker exec -it auto-sign-machine /bin/sh
vi /etc/crontabs/root  # 或者 crontab -e
30 */2 * * * node index.js unicom --user *** --password *** --appid *** > /AutoSignMachine/**.log
```
### 注意
#### cron中`%`号需要转义`\%`
...
每隔10分钟尝试运行可运行的脚本子任务
```txt
*/10 * * * * /bin/node /AutoSignMachine/index.js unicom --user 1******5 --password 7****** --appid 1************9
```
